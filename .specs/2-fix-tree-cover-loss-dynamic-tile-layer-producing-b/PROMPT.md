# Feature Spec: Fix Tree Cover Loss dynamic tile layer producing blank map (no pink pixels)

## Requirements

## Job Story
When Zeno responds to a forest loss query with a named AOI, I want to see Tree Cover Loss pixels (pink/red) rendered on the map overlaid on the basemap, so I can visually verify the forest loss extent and spatial distribution for the queried year.

## Promise
After this ships: querying "How much forest was lost in [country] in [year]?" renders a visible TCL tile layer with correctly colored pixels over the AOI, filtered to the queried year range. The root cause is identified and fixed in a way that generalizes to other GFW-sourced tile layers, not just patched for TCL alone.

## Constraints
- Must use the existing dynamic tile URL generation path in `pick_dataset.py` — no hardcoded tile URLs in the frontend
- The investigation must compare how Grasslands (working) vs. TCL (broken) differ in URL construction, tile provider, and rendering — the fix must address the root cause, not patch symptoms
- Must not regress other dataset visualizations (Grasslands, DIST-ALERT, etc.)
- Analytics data (statistics, charts) must continue to work independently of the map fix
...
## Acceptance Criteria
- [ ] Querying forest loss for a named country renders TCL pink/red pixels on the map for the specified year range
- [ ] The tile URL generated by `pick_dataset.py` for TCL produces a valid, tile-serving response (verifiable in browser or curl)
- [ ] Other dataset tile layers (Grasslands, DIST-ALERT) continue to render correctly after the fix
- [ ] The fix works for any valid year within TCL coverage (2001–2024), not just 2024
- [ ] Root cause is documented in the PR so other GFW-sourced datasets can be checked against the same failure mode
- [ ] Layer order is correct for all map renders: basemap at bottom, dataset tile layer(s) in the middle, AOI outline(s) on top

## Context
Dataset tile layers are loaded dynamically via `src/agent/tools/pick_dataset.py`. The agent uses RAG + LLM to select the best dataset, then constructs a parameterized tile URL that the frontend renders via `folium.raster_layers.TileLayer`.

Two datasets behave differently:
- **Global Grasslands** (working): uses a relative URL served by the project's own eoAPI backend (`/raster/collections/grasslands-v-1/...`), with a `{year}` path template that gets substituted.
- **Tree Cover Loss** (broken): uses an absolute GFW tile service URL (`https://tiles.globalforestwatch.org/umd_tree_cover_loss/...`) with `start_year` and `end_year` appended as query parameters.

The TCL tile layer renders a blank overlay despite the agent correctly selecting the dataset and generating a URL. Grasslands renders correctly. The investigation should determine whether the issue is in the tile URL construction, CORS/network access to the GFW tile service, the `render_type` parameter, or the year range parameters.

Note: a recent commit (`b7bfe83`) already capped `start_year` at 2023 for the GFW tile service — this context may be relevant to the investigation.
...
> to `/mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/images/`. Sub-agents should use the Read tool to examine any images when relevant.
...
- Read the /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md file that the orchestrator provides in your working directory to understand what feature is being planned.
- Explore the actual codebase using Glob, Grep, and Read tools to understand:
  ...
  - If /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/images/ exists, use the Read tool to examine any images for visual context (screenshots, mockups, diagrams)
- Write your findings to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md
...
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md"
...
- Write your findings to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Include actual code snippets, function signatures, and type definitions. Reference specific file paths and line numbers.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Write your findings to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Include concrete examples from the codebase. Reference specific file paths.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```

**After all 3 complete:** Confirm all 3 files exist (/mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md), then proceed to Stage 1. Do NOT read the files.
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md for full requirements.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, and /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md for codebase context.
...
- Write a detailed, complete implementation plan to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_1.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
...
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_1.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Write a detailed, complete implementation plan to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_2.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Cover: architecture, specific file changes, data models, API design, error handling, testing strategy, migration plan.
- Be specific — reference actual file paths, function names, and code patterns from the recon documents.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_2.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Write a detailed, complete implementation plan to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_3.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Cover: architecture, specific file changes, data models, API design, error handling, testing strategy, migration plan.
- Be specific — reference actual file paths, function names, and code patterns from the recon documents.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_3.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Write a detailed, complete implementation plan to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_4.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Cover: architecture, specific file changes, data models, API design, error handling, testing strategy, migration plan.
- Be specific — reference actual file paths, function names, and code patterns from the recon documents.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_4.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md for the full requirements.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, and /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md for codebase context.
- Read ALL of: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_1.md through /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_4.md (4 drafts total).
...
- Write your evaluation to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/evaluation.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/evaluation.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md for the full requirements.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, and /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md for codebase context.
- Read ALL of: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_1.md through /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/draft_plan_4.md (4 drafts).
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/evaluation.md (the rubric scores and comparative analysis).
...
- Write the master plan to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/master_plan.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/master_plan.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md for the full requirements.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/master_plan.md — this is the plan you must audit.

YOUR MISSION: Systematically verify every requirement from the original issue.
For EACH requirement in /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md:
...
- Write your audit to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_1.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_1.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md for the full requirements.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, and /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md for codebase context.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/master_plan.md — this is the plan you must attack.
...
- Write your findings to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_2.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_2.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/master_plan.md — this is the plan you must validate.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, and /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md for codebase context.
...
- Write your validation report to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_3.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_3.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Write your findings to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_4.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_4.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```
...
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/PROMPT.md for the full requirements.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/architecture.md, /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/relevant_code.md, and /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/recon/conventions.md for codebase context.
- Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/master_plan.md (the master plan).
- Read ALL of: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_1.md through /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/red_team_4.md (4 red team reports).
...
- Write the final spec to /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/SPEC.md
- Write in chunks of ~4000 tokens maximum. Use Write tool first, then Edit tool to append.
- Your final response must be ONLY: "Done. Output: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/SPEC.md"
- Do NOT return any content, summaries, or explanations. ONLY the done message.
```

**After completion:** Read /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/SPEC.md and present it to the user. This is the ONLY file you read.
...
- NEVER read sub-agent output files yourself (the ONLY exception: /mnt/e/agentdev/projects/project-zeno/.specs/2-fix-tree-cover-loss-dynamic-tile-layer-producing-b/plans/SPEC.md at the very end)
